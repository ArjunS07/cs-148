...
\begin{itemize}
    ...
    \item[(f)] \begin{enumerate}
        \item ResNet56 (no short) has a much more complicated loss landscape than ResNet20 (no short), i.e., it has a significantly more sophisticated topology with many more local minima and maxima; this is due to the increased parameter space of the larger model
        \item Resnet56 (short) has a smoother loss landscape than ResNet 56 (no short); intuitively, this may be because the introduction of residual connections means that adding more parameters around some minima at worst has no impact on performance, so there are fewer peaks and local minima which result from additional complexity in the network.
    \end{enumerate}
\end{itemize}
...
\begin{itemize}
    \item[(b)] 
        ...
        \item[(c)] \begin{enumerate}[label=(\roman*)]
