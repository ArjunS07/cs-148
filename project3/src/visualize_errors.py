...
Usage:
    python visualize_errors.py checkpoints/run1/best_model.pt
"""
...
def plot_misclassified_grid(results, out_dir, max_per_image: int = 8):
    ...
    n = min(len(errors), max_per_image * 4)
    if n == 0:
...
def plot_errors_by_class(results, out_dir, max_per_class: int = 6):
    ...
    for c in range(NUM_CLASSES):
        ...
        for j in range(max_per_class):
            ...
            ax.axis("off")
            if j == 0:
                ax.set_ylabel(f"True={c}", fontsize=10, rotation=0, labelpad=40, va="center")
                ax.axis("on")
                ax.set_xticks([])
                ax.set_yticks([])
            ...
            if j < len(errs):
                ...
                img = denormalize(e["image"]).permute(1, 2, 0).numpy()
                ax.imshow(img)
                ax.set_title(f"->{e['pred']} ({e['confidence']:.2f})", fontsize=8, color="red")
    ...
    plt.close(fig)
    print("  misclassified_by_class.png")
...
def save_misclassified_images(results, out_dir):
    ...
    for i, e in enumerate(errors):
        ...
        ax.imshow(img)
        ax.set_title(
            f"True: {e['true']}  Pred: {e['pred']}  Conf: {e['confidence']:.2f}", fontsize=10
        )
        ax.axis("off")
        fig.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="Visualize ViT misclassified examples")
    ...
    print(f"  {len(results)} samples, {n_errors} errors ({n_errors/len(results)*100:.1f}%)")
