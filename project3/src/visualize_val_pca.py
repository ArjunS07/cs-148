...
Usage:
    python visualize_val_pca.py checkpoints/run1/best_model.pt
"""
...
CORRECT_COLOR = sns.color_palette("deep")[2]
INCORRECT_COLOR = sns.color_palette("deep")[3]
...
def plot_per_class_pngs(coords, labels, preds, confs, out_dir, method):
    ...
    for c in range(NUM_CLASSES):
        ...
        cm = mask & correct
        if cm.any():
            ax.scatter(coords[cm, 0], coords[cm, 1], color=[CORRECT_COLOR],
                       s=15, alpha=0.5, label=f"Correct ({n_correct})")
        wm = mask & ~correct
        if wm.any():
            ax.scatter(coords[wm, 0], coords[wm, 1], color=[INCORRECT_COLOR],
                       s=25, alpha=0.8, marker="x", linewidths=1.5,
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  #sidebar { width: 200px; display: flex; flex-direction: column; align-items: center; gap: 8px; padding-top: 20px; }
  #hover-img { width: 160px; height: 160px; object-fit: contain; border: 2px solid #bbb; border-radius: 4px; background: #eee; display: none; }
  #hover-label { font-size: 13px; text-align: center; min-height: 60px; white-space: pre-line; }
...
const pages = [];
for (let c = 0; c < 10; c++) pages.push({id: 'class_' + c, title: 'Class ' + c});
...
function plotClass(cls) {
  ...
  const traces = [];
  if (correctIdx.length > 0) traces.push({
    x: correctIdx.map(i => d.x[i]), y: correctIdx.map(i => d.y[i]),
    customdata: correctIdx, mode: 'markers', type: 'scattergl',
    marker: {size: 5, color: COLOR_CORRECT, opacity: 0.5},
    name: 'Correct (' + correctIdx.length + ')',
  });
  if (incorrectIdx.length > 0) traces.push({
    x: incorrectIdx.map(i => d.x[i]), y: incorrectIdx.map(i => d.y[i]),
    customdata: incorrectIdx, mode: 'markers', type: 'scattergl',
...
def generate_dashboard(coords, data, method, out_path):
    ...
    payload = {
        ...
        "confs": data["confs"],
        "thumbs": data["thumbs"],
        "xlim": [float(coords[:, 0].min() - pad_x), float(coords[:, 0].max() + pad_x)],
        "ylim": [float(coords[:, 1].min() - pad_y), float(coords[:, 1].max() + pad_y)],
    }
    html = HTML_TEMPLATE.replace("__DATA_JSON__", json.dumps(payload))
    with open(out_path, "w") as f:
        f.write(html)
    print(f"  {out_path} ({len(html) / 1024 / 1024:.1f} MB)")
...
def main():
    ...
    model.eval()
    print(f"Loaded ViT from {args.checkpoint} (epoch {ckpt['epoch']}, val_acc={ckpt['val_acc']:.4f})")
